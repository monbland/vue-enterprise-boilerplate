...
# Where to find files for extraction
INPUT_DIR = src/components/Nova
...
GETTEXT_HTML_SOURCES = $(shell find $(INPUT_DIR) -name '*.vue' -o -name '*.html' | grep -v "$(INPUT_DIR)/vendor/.*" 2> /dev/null)
GETTEXT_JS_SOURCES = $(shell find $(INPUT_DIR) -name '*.vue' -o -name '*.js' | grep -v "$(INPUT_DIR)/vendor/.*")
...
makemessages: removetemplate /tmp/template.pot
...
# Extract gettext strings from templates files and create a POT dictionary template.
	gettext-extract --attribute v-translate  --startDelimiter "" --endDelimiter "" --quiet --output $@ $(GETTEXT_HTML_SOURCES)
